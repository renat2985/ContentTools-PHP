...
editor.addEventListener('saved', function (ev) {
    ...
    for (name in regions) {
    // $.post( "test.php", { name: name, content: regions[name] } );



     $.post( "admin.php?file="+window.location.pathname.split('/')[window.location.pathname.split('/').length-1], { name: name, content: regions[name]})
  .done(function( data ) {
     editor.busy(false);
